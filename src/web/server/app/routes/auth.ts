...
import {isLoggedIn, isLoggedOut, isUsernameCollide, isValidCred} from '../middlewares/authChecks';
...
app.post(
    '/register', [isLoggedOut, isValidCred, isUsernameCollide],
...
app.post(
    '/login', [isLoggedOut, isValidCred],
    (req: express.Request, res: express.Response) => {
      const {username, password} = req.body;
      const sid = req.sessionID;

      client.execute(
          queries.SELECT_USERSID_USERID_PASSWORD, [username], {prepare: true},
            ...
            if (err) {
              ...
              bcrypt.compare(password, hash, function(error, match) {
                if (error) {
                  ...
                  client.execute(
                      queries.UPDATE_USERSID_SID, [sid, id], {prepare: true});
                  client.execute(
                      queries.UPDATE_USERSNAME_SID, [sid, username],
                      {prepare: true});
...
app.get('/rtmpAuth', async (req, res) => {
  const result = await client.execute(
      queries.SELECT_USERSNAME_SID, [req.body.userId], {prepare: true});
