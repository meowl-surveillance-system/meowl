...
def draw_box(writer, args, start, end, layerOutputs,
    ...
    if len(objs.keys()) > 0:
        ...
        #cv_producer.send_detections(json.dumps(objs), img_str)
        cv_producer.send_detections(args['camera_id'], args['stream_id'], frame_id, json.dumps(objs))
        store_frame.insert_frame(repr(args['camera_id']), repr(args['stream_id']), repr(frame_id), img_str, json.dumps(objs).encode('utf-8'))
        return 1
    return 0
    #writer.write(frame)
